...
bool robot::avoid_crash(robot bot[], int now_frame)
{
    ...
    for (int i = 0; i < 4; i++)
        memcpy(&temp_bot[i], &bot[i], sizeof(robot_data));
    for (int i = 0; i < 4; i++)
    {
        if (i == data.num)
            continue;
        ...
        if (now_frame >= 750 && distance <= 3 && (cos(data.toward) * cos(temp_bot[i].toward) + sin(data.toward) * sin(temp_bot[i].toward)) <= -0.8 && temp_bot[i].rest_crash == 0)
        {
            ...
            {
                if (temp_bot[i].toward > 0)
                    temp_bot[i].toward = temp_bot[i].toward - 3.1415926;
                else
                    temp_bot[i].toward = 3.1415926 + temp_bot[i].toward;
